...
def load_reference_tables(csv_file_path):
    ...
    ingredient_ref_table = reference_table[reference_table['type'].isin(['Coffee'])].sort_values(['type', 'name']).drop(columns=['type'])
...
def main():
    ...
    ingredient_ref_table["display"] = (
        ingredient_ref_table["name"]
        + " ------------ "
        + ingredient_ref_table["vendor"]
        + " ------------ "
    ...
    with st.expander('Edit additional details', icon='✏️'):
        ...
        if "entries" not in st.session_state:
            st.session_state.entries = [
                {"product_name": "Cafelat Robot ------------ Cafune"},
                {"product_name": "Robot Paper Filters ------------ Cafune"},
                {"product_name": "Subminimal Flick WDT Tool ------------ Cafune"},
                {"product_name": "notNeutral VERO Espresso Glass ------------ Eight Ounce Coffee"},
            ]

        def add_entry():
            st.session_state.entries.append({"product_name": ""})
        ...
        equip_ref_table["display"] = (
            equip_ref_table["name"]
            + " ------------ "
            + equip_ref_table["vendor"]
        )


        #ingredient = st.selectbox(
        #    "Equipment",
        #    equip_ref_table["display"].tolist(),
        #    index=1
        #)

        options = [""] + equip_ref_table["display"].tolist()

        for i, entry in enumerate(st.session_state.entries):
            idx = (
                options.index(entry["product_name"])
                if entry.get("product_name") in options
                else 0
            )

            # get selection from selectbox
            ...
            # split into product_name and vendor_name
            if " ------------ " in selection:
                entry["product_name"], entry["vendor_name"] = selection.split(" ------------ ", 1)
            else:
                entry["product_name"], entry["vendor_name"] = selection, ""
        ...
        cols_order = [
            ...
            'quantity_used', 'quantity_output', 'unit'
        ]
    ...
    if st.button('Add new rows to csv(s)', width='stretch', type='primary'):
        ...
        # set flag to show message on next run
